...
class StatsController extends Controller
{
    ...
    public function summary(): void
    {
        ...
        $nr_users = (int)$pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();
        $nr_campings = (int)$pdo->query("SELECT COUNT(*) FROM campings WHERE approval_status = 1")->fetchColumn();
        $nr_pending  = (int)$pdo->query("SELECT COUNT(*) FROM campings WHERE approval_status = 0")->fetchColumn();

        $bookings_by_status = $pdo->query(
        ...
        $top_campings = $pdo->query(
            "SELECT c.id, c.name, ROUND(AVG(r.rating)::numeric,2) AS avg_rating, COUNT(r.id) AS nr_reviews
             ...
             ORDER BY avg_rating DESC, nr_reviews DESC LIMIT 5"
        )->fetchAll(PDO::FETCH_ASSOC);

        $bookings_per_month = $pdo->query(
        ...
        $this->json([
            'nr_users'           => $nr_users,
            'nr_campings'        => $nr_campings,
            'nr_pending'         => $nr_pending,
            'bookings_by_status' => $bookings_by_status,
            'total_revenue'      => $total_revenue,
            'top_regions'        => $top_regions,
            'top_campings'       => $top_campings,
    ...
    public function chartSvg(): void
    {
        ...
        if ($type === 'bookings_per_month') {
            ...
            $title = 'Rezervari / luna (12 luni)';
        } elseif ($type === 'top_regions') {
            $rows = $pdo->query(
            ...
            $title = 'Campinguri pe regiune';
        } elseif ($type === 'revenue_per_month') {
            $rows = $pdo->query(
                "SELECT TO_CHAR(created_at,'Mon YY') AS label, COALESCE(SUM(total_price),0) AS value
                 FROM bookings WHERE status='confirmed' AND created_at >= NOW() - INTERVAL '12 months'
                 GROUP BY TO_CHAR(created_at,'YYYY-MM'), TO_CHAR(created_at,'Mon YY')
                 ORDER BY MIN(created_at)"
            )->fetchAll(PDO::FETCH_ASSOC);
            $title = 'Venituri confirmate / luna (RON)';
        } else {
            $this->json(['error' => 'type invalid'], 400);
        }

        header('Content-Type: image/svg+xml; charset=utf-8');
        echo $this->buildBarChart($rows, $title);
        exit();
    }
    ...
    {
        $W = 700; $H = 340;
        $padL = 54; $padR = 16; $padT = 48; $padB = 60;
        $chartW = $W - $padL - $padR;
        $chartH = $H - $padT - $padB;
        ...
        $maxVal = max($values) ?: 1;
        $n = count($rows);
        $barW = max(8, ($chartW / $n) * 0.6);
        $gap  = $chartW / $n;

        $bars  = '';
        $xLabels = '';
        $yStep = $this->niceStep($maxVal, 5);
        ...
        for ($i = 0; $i < $n; $i++) {
            ...
            $y    = $padT + $chartH - $bH;
            $bars .= "<rect x='" . round($x,1) . "' y='" . round($y,1)
                   . "' width='" . round($barW,1) . "' height='" . round($bH,1)
                   . "' rx='3' fill='#D97706' opacity='0.85'/>";
            $bars .= "<text x='" . round($x + $barW/2,1) . "' y='" . round($y-4,1)
                   . "' text-anchor='middle' font-size='10' fill='#555'>"
                   . ($val >= 1000 ? round($val/1000,1).'k' : round($val)) . "</text>";
            $xLabels .= "<text x='" . round($x + $barW/2,1) . "' y='" . ($padT+$chartH+16)
        ...
        for ($v = 0; $v <= $maxVal; $v += $yStep) {
            ...
            $yLines .= "<line x1='$padL' x2='" . ($W-$padR) . "' y1='" . round($y,1) . "' y2='" . round($y,1)
                    . "' stroke='#e5e7eb' stroke-width='1'/>";
            $yLines .= "<text x='" . ($padL-6) . "' y='" . round($y+4,1)
                    . "' text-anchor='end' font-size='10' fill='#888'>$label</text>";
        }

        return "<?xml version='1.0' encoding='UTF-8'?>"
             . "<svg xmlns='http://www.w3.org/2000/svg' width='$W' height='$H' style='font-family:sans-serif;background:#fff'>"
             . "<text x='" . ($W/2) . "' y='28' text-anchor='middle' font-size='14' font-weight='600' fill='#1f2937'>"
             . htmlspecialchars($title, ENT_XML1) . "</text>"
             . $yLines . $bars . $xLabels
    ...
    public function reportPdf(): void
    {
        ...
        $nr_campings = (int)$pdo->query("SELECT COUNT(*) FROM campings WHERE approval_status = 1")->fetchColumn();
        $nr_pending  = (int)$pdo->query("SELECT COUNT(*) FROM campings WHERE approval_status = 0")->fetchColumn();
        $total_rev   = (float)$pdo->query("SELECT COALESCE(SUM(total_price),0) FROM bookings WHERE status='confirmed'")->fetchColumn();
        $nr_bookings = (int)$pdo->query("SELECT COUNT(*) FROM bookings")->fetchColumn();

        $top_campings = $pdo->query(
            "SELECT c.name, ROUND(AVG(r.rating)::numeric,2) AS avg_rating, COUNT(r.id) AS nr_reviews
             FROM campings c JOIN reviews r ON r.camping_id = c.id
             WHERE c.approval_status = 1
             GROUP BY c.id, c.name HAVING COUNT(r.id) >= 1
...
<style>
  ...
  .card { display: table-cell; padding: 12px 16px; background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 6px; width: 22%; text-align: center; margin-right: 8px; }
...
<table>
  <tr><th>#</th><th>Camping</th><th>Rating Mediu</th><th>Recenzii</th></tr>
  $topRows
</table>
