...
class OrganizersRepository extends Repository
{
    ...
    public function findPendingApplications(int $limit = 20, int $offset = 0): array
    {
        ...
        $stmt   = $this->pdo->prepare(
            "SELECT ov.*, u.username, u.email FROM organizer_verifications ov
             ...
             WHERE ov.status = 'pending' ORDER BY ov.submitted_at ASC LIMIT $limit OFFSET $offset"
        );
        $stmt->execute();
    ...
    public function approveApplication(int $appId, int $reviewerId): bool
    {
        ...
        try {
            ...
            if (!$userId) { $this->pdo->rollBack(); return false; }
            $this->pdo->prepare("UPDATE users SET role = 'organizer' WHERE id = :id")->execute(['id' => $userId]);
            $this->pdo->commit();
            return true;
        } catch (Exception $e) {
            $this->pdo->rollBack();
    ...
    public function rejectApplication(int $appId, int $reviewerId, string $notes): bool
    {
        $stmt = $this->pdo->prepare(
            "UPDATE organizer_verifications
             SET status = 'rejected', reviewed_by = :reviewer, reviewed_at = NOW(), admin_notes = :notes
             WHERE id = :id AND status = 'pending'"
        );
        $stmt->execute(['id' => $appId, 'reviewer' => $reviewerId, 'notes' => $notes]);
